...
1. **Clon de huella** (`levels/fingerprint.ts`) — la huella objetivo esta a la
   ...
   Las huellas se generan proceduralmente (`makeFingerprint`, función de crestas
   basada en seno) y se pintan pixel a pixel en canvas.
2. **Decodificador** (`levels/decoder.ts`) — un codigo de 4 numeros de dos digitos
   ...
   El cursor arrastra una ventana de 4 celdas; Enter/click la valida. Encontrar
   la corrida correcta (`CODES` = 1) pasa el nivel.
3. **BruteForce** (`levels/bruteforce.ts`) — la clave (una palabra del pool) se
   muestra arriba; abajo hay un reel vertical por letra que gira por el alfabeto.
   Alinear la banda central de cada reel con la letra objetivo lo fija (auto-lock,
   sin confirmar). Todos fijados = nivel resuelto.
...
nivel activo; cada nivel maneja sus propios clicks/pointer sobre su DOM.
